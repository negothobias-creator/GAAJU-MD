...
const { handleAutoReplyOnTag } = require('../plugins/autoreply');
const { isAntiGroupMentionEnabled } = require('../plugins/antigroupmention');
const { handleChannelDetection } = require('../plugins/antichannel');
const { handleAdvertiseDetection } = require('../plugins/antiadvertise');
const { isAntibotEnabled } = require('../plugins/antibot');
...
// default channelInfo added to command replies.  We intentionally keep this
// empty so that commands do not carry any forwarding or newsletter metadata.
// This effectively strips out all channel promotion from the bot's responses.
const channelInfo = {};
...
async function handleMessages(sock, messageUpdate) {
    try {
        ...
        // Handle autoreply on tag (always check)
        if (isGroup) {
            const botNumber = sock.user.id.split(':')[0] + '@s.whatsapp.net';
            await handleAutoReplyOnTag(sock, message, botNumber);
        }
        ...
        if (!message.key.fromMe) {
            ...
            // Record user activity for kickinactive feature
            if (global.recordUserActivity && isGroup) {
                global.recordUserActivity(chatId, senderId);
            }
        }
        ...
        if (isGroup) {
            if (userMessage) {
                ...
                await handleChannelDetection(sock, chatId, message, userMessage, senderId);
                await handleAdvertiseDetection(sock, chatId, message, userMessage, senderId);
            }
            await handleLinkDetection(sock, chatId, message, userMessage, senderId);

            // Handle anti‑group‑mention detection (delete messages or status with @group mentions)
            const antibadwordEnabled = await isAntiGroupMentionEnabled(chatId);
            if (antibadwordEnabled) {
                const groupMentionPattern = /@group|@everyone|@here|\[GROUP\]/gi;
                let shouldDelete = groupMentionPattern.test(userMessage);
                
                // Check if it's a status message mentioning the group
                if (message.message?.statusMessage) {
                    const statusText = message.message.statusMessage.text || '';
                    if (groupMentionPattern.test(statusText.toLowerCase())) {
                        shouldDelete = true;
                    }
                }
                ...
                if (shouldDelete) {
                    try {
                        ...
                        printLog('warning', `🚫 Group mention/status deleted from ${senderId.split('@')[0]}`);
                    } catch (error) {
                        console.error('Error deleting group mention message:', error);
                    }
                }
            }
        }
        ...
        switch (action) {
            ...
            case 'add':
                if (participants && participants.length > 0) {
                    ...
                    // Check if antibot is enabled and auto-kick if new participant is a bot
                    try {
                        const antibotEnabled = await isAntibotEnabled(id);
                        if (antibotEnabled && typeof participant === 'string') {
                            // Get metadata to check if it's a bot
                            const metadata = await sock.groupMetadata(id);
                            const joinedUser = metadata.participants.find(p => p.id === participant);
                            
                            if (joinedUser) {
                                const isBot = joinedUser.name?.includes('Bot') || 
                                             joinedUser.name?.includes('bot') ||
                                             joinedUser.id.startsWith('212') ||
                                             !joinedUser.name;
                                
                                if (isBot && participant !== sock.user?.id) {
                                    try {
                                        await sock.groupParticipantsUpdate(id, [participant], 'remove');
                                        const { incrementBotKickCount } = require('../plugins/antibot');
                                        await incrementBotKickCount(id);
                                        printLog('success', `🤖 Bot auto-kicked: ${participant.split('@')[0]}`);
                                    } catch (kickError) {
                                        printLog('error', `Error kicking bot: ${kickError.message}`);
                                    }
                                }
                            }
                        }
                    } catch (antibotError) {
                        console.error('Error in antibot detection:', antibotError);
                    }
                }
