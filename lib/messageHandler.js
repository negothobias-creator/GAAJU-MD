...
async function handleMessages(sock, messageUpdate) {
    try {
        ...
        if (isGroup) {
            ...
            // Handle antimention detection (delete messages with @group mention)
            ...
            if (antibadwordEnabled) {
                ...
                if (groupMentionPattern.test(userMessage)) {
                    try {
                        await sock.sendMessage(chatId, {
                            delete: message.key
                        });
                        printLog('warning', `🚫 Group mention deleted from ${senderId.split('@')[0]}`);
                    } catch (error) {
        ...
        if (!usedPrefix && !command) {
            ...
            if (isGroup) {
                // Handle autoreply on tag (before other detections)
