...
function initSecurityFile() {
  if (!fs.existsSync(SECURITY_FILE)) {
    const defaultSecurity = {
      mainOwner: null,
      authorizedUsers: [],
      // default PIN has been changed to 1983 for this bot
      restartPin: process.env.RESTART_PIN || '1983',
      securityLog: [],
      failedAttempts: {},
      lockdownMode: false,
      blacklistedUsers: []
...
function readSecurity() {
  try {
    ...
    // if an old default PIN of '0000' is present, migrate to new default
    if (data.restartPin === '0000') {
      data.restartPin = process.env.RESTART_PIN || '1983';
      writeSecurity(data);
    }
    return data;
  } catch (err) {
